<template>
  <div class="nova-translation">
    <div
      ...
      class="flex space-x-1 divide-x divide-gray-400 dark:divide-gray-700"
      v-if="field.value"
    >
      <template v-for="otherLocale in otherLocales" :key="`translation_${otherLocale?.id}`">
        <div v-if="isTranslated[otherLocale?.id] ?? false" class="pl-1 first:pl-0">
          <Link :href="translatedDetailRoute(otherLocale)" class="link-default" @click.stop>

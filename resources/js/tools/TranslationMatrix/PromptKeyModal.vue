<template>
  ...
  >
    ...
    >
      <div class="space-y-6">
        <ModalHeader>{{ trans('Add a translation key', {}) }}</ModalHeader>

        <div class="action">
          <div class="flex flex-col md:flex-row">
            <div class="px-6 md:px-8 mt-2 md:mt-0 w-full md:w-1/5 md:py-5">
              <label for="newKey-default-text-field" class="inline-block pt-2 leading-tight"
                >Key <span class="text-red-500 text-sm">*</span></label
              >
            </div>
      ...
      <ModalFooter>
        <div class="flex items-center ml-auto">
          <Button variant="link" state="mellow" @click.prevent="handleClose" class="mr-3">
            {{ trans('Cancel', {}) }}
          </Button>

          <Button type="submit" :loading="false" variant="solid" state="default">
            {{ trans('Confirm', {}) }}
          </Button>
        </div>
      </ModalFooter>
